...
impl PaymentSetup {
    pub fn new(config: &Config) -> Result<Self, PaymentError> {
        ...
        for chain_config in &config.chain {
            ...
            ps.chain_setup.insert(chain_config.1.network_id, ChainSetup {
                providers,
    ...
    pub fn get_chain_setup(&self, chain_id: i64) -> Result<&ChainSetup, PaymentError>{
        self.chain_setup.get(&(chain_id as usize)).ok_or(
    ...
    pub fn get_provider(&self, chain_id: i64) -> Result<&Web3<Http>, PaymentError> {
        let chain_setup = self.chain_setup.get(&(chain_id as usize)).ok_or(
            PaymentError::OtherError(format!("No chain setup for chain id: {}", chain_id)
        ))?;

        let provider = chain_setup.providers.get(0).ok_or(
